...
ostream& operator << (ostream& stream, const GenericMatrix<T>& mtrx)
{
    ...
    stream << endl;
    stream << "Матрица №" << mtrx.getIdentity()
...
{
    this->n = n > 0 ? n : throw "Значение размерности строк должно быть положительным";
}
...
{
    this->m = m > 0 ? m : throw "Значение размерности столбцов должно быть положительным";
}
...
T* const GenericMatrix<T>::operator [] (int i) const
{
    ...
    {
        throw "Выход индекса за допустимый диапазон!";
    }
}
...
GenericMatrix<T> GenericMatrix<T>::power(int n) const
{
    if (n <= 1)
    {
        throw "Значение степени для матрицы должно быть больше или равно 1.";
    }
